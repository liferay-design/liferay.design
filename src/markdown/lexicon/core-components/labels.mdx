---
title: 'Labels'
titleLabel: 'Clay'
titleLabelLink: 'https://clayui.com/docs/components/label.html'
description: 'Labels are a visual pattern used to categorize information providing quick and easy recognition.'
order: 281
draft: false
---

import { Grid } from 'reakit'

### Overview

Labels can be applied in a wide range of cases as a mechanism to tag content, show status, filter content or even provide navigation to elements of the same identification.

![labels](/images/lexicon/Labels.png)


### Simple labels
Use simple labels to identify or filter content, tag content, show status or even to provide navigation to elements.

![Simple labels](/images/lexicon/SimpleLabel.png)

Simple labels can use status colors to highlight the status of an element to provide quick recognition. Simple status labels use colors to indicate meaning and they can be bold or outline. The colors are chosen intentionally so that users can recognize them easily.


#### Outline status labels

Use outline status labels by default and in places where there are many status labels on the same screen such as in tables, lists...etc.

![Outline status labels](/images/lexicon/OutlineStatusLabels.png)

In the following image, you can see different status labels in a table. In this example, outline status labels are used as they dominate the screen.
<br/>

![Outline status labels in a table](/images/lexicon/TableWithLabels.png)


#### Bold status labels

Bold status labels are used in an infrequent manner to draw attention to important information. For example, in headers when you need to differentiate a label from the rest of the labels in a list. Do not use bold labels as default or alongside many other status labels. Background color and text in bold status labels have a **minimum contrast ratio of 5:1** for each color.

![Bold status labels](/images/lexicon/BoldStatusLabels.png)

In the image below, you can see different status labels in a table and a status label in the header section. In this example, a bold status label is used in the header to differentiate it from other labels.
<br/>

![Bold status labels in header and outline status labels in a table](/images/lexicon/BoldStatusLabelInHeader.png)

### Removable label

Use the removable label in filter bars, categories, etc. A removable label should always be the default size and have no status color. It contains a close icon to remove the label.

![label removable](/images/lexicon/LabelRemovable.png)

#### Removable label with sticker

This label is a variation from label removable, which can only be used with a default size. You can use this label with icons or round stickers.

![Removable label with sticker](/images/lexicon/RemovableLabelSticker.png)

The label must never be longer than the available space. However, there are cases in which removable labels need to be placed in a second line or as a last option it will be truncated.

![Removable labela with sticker break line](/images/lexicon/RemovableLabelStickerBreakLine.png)

### Label with link

You can use labels to link related content to a site.

![default size default color link label](/images/lexicon/LabelLink.jpg)

### Sizes

Labels can have two different sizes:

| Size                                                            | Usage                                                                                                                                                                                           |
| --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![default label small size](/images/lexicon/LabelSmall.png)     | **Small**: Use the small label when the label doesn’t need emphasis. Always use uppercase text for the small label. Small labels are usually used in tables, list and cards.  |
| ![default label default size](/images/lexicon/LabelDefault.png) | **Default**: Use the default size to mark states. The use of lowercase is acceptable for this size. We recommend using the default label outside of tables like for example in headers.         |


### Dos and Don'ts

1. Labels categorize information and provide quick recognition. They do not contain badges.

<Grid templateColumns="1fr 1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/LabelSmallDo.jpg"
				alt="Simple label"
			/>
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
		<img
			src="/images/lexicon/LabelSmallDont.jpg"
			alt="Label with sticker inside"
		/>
		</div>
		<p class="dont">Don't</p>
	</Grid.Item>
</Grid>
<br/>
<br/>

2. Labels can only contain one line of text.

<Grid templateColumns="1fr 1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/LabelTextDo.jpg"
				alt="default label"
			/>
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/LabelTextDontLines.jpg"
				alt="default label with 2 lines of text"
			/>
			</div>
		<p class="dont">Don't</p>
	</Grid.Item>
</Grid>
<br/>
<br/>

3. Never use Ellipsis on labels.

<Grid templateColumns="1fr 1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img src="/images/lexicon/LabelTextDo.jpg" alt="default label" />
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/LabelTextDontEllipsis.jpg"
				alt="default label with ellipsis"
			/>
		</div>
		<p class="dont">Don't</p>
	</Grid.Item>
</Grid>
<br/>
<br/>

4. Keep your text short, up to 1 or 2 words.

<Grid templateColumns="1fr 1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img src="/images/lexicon/CollectionLabel.png" alt="default label" />
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/CollectionLabelLong.png"
				alt="default label long"
			/>
		</div>
		<p class="dont">Don't</p>
	</Grid.Item>
</Grid>
<br/>
<br/>

5. Don’t use bold status labels alongside many other status labels.

<Grid templateColumns="1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img src="/images/lexicon/StatusLabelsTableDo.png" alt="Outline Status Labels in a Table" />
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/BoldLabelsTableDont.png"
				alt="Bold Status Labels in a Table Dont"
			/>
		</div>
		<p class="dont">Don't</p>
	</Grid.Item>
</Grid>
<br/>
<br/>

6. Use color labels for status only.

<Grid templateColumns="1fr 1fr" autoRows="auto" gap="2rem">
	<Grid.Item>
		<div class="image-container">
			<img src="/images/lexicon/SuccessLabel.png" alt="SuccessLabel" />
		</div>
		<p class="do">Do</p>
	</Grid.Item>
	<Grid.Item>
		<div class="image-container">
			<img
				src="/images/lexicon/StatusLabelsNotStatus.png"
				alt="Bold Status Labels in a Table Dont"
			/>
		</div>
		<p class="avoid">Avoid</p>
	</Grid.Item>
</Grid>